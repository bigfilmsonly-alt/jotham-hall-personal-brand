...
const features = [
  {
    ...
    description: "Technology that replaces repetition, reduces overhead, and scales output without scaling stress.",
    visual: "ai",
  },
  {
    number: "02",
    title: "Revenue System Design",
    description: "Infrastructure that turns attention into income. Stop guessing, start engineering growth.",
    visual: "deploy",
  },
  {
    number: "03",
    title: "Brand Positioning",
    description: "Clarity that commands premium pricing. Articulate what you do and why it matters.",
    visual: "collab",
  },
  {
    number: "04",
    title: "Operational Efficiency",
    description: "Find the 20% producing 80% of results, then systematize it. Architecture, not cliches.",
    visual: "security",
  },
];
  ...
  return (
    <svg viewBox="0 0 200 160" className="w-full h-full">
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        const radius = 50;
        const x2 = Math.round((100 + Math.cos(angle) * radius) * 100) / 100;
        const y2 = Math.round((80 + Math.sin(angle) * radius) * 100) / 100;
        return (
          <g key={i}>
            {/* Connection line */}
            <line
...
function FeatureCard({ feature, index }: { feature: typeof features[0]; index: number }) {
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-16 py-8 lg:py-20 border-b border-foreground/10 text-center lg:text-left px-2 lg:px-0">
        {/* Number */}
        <div className="shrink-0">
          <span className="font-mono text-xs lg:text-sm text-muted-foreground">{feature.number}</span>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="mb-16 lg:mb-24 text-center lg:text-left">
          ...
          >
            ...
            <br />
            <span className="text-muted-foreground">Architect your advantage.</span>
          </h2>
        </div>
